...
<body>
  ...
  <script>
    ...
    function render(){const query=keyword.value.trim().toLowerCase(),state=status.value;const rows=items.filter(item=>(!state||item.status===state)&&(!query||[item.contract_number,item.title].join(' ').toLowerCase().includes(query)));
      ...
      content.className='';content.innerHTML='<table><thead><tr><th>合同编号 / 标题</th><th>类型</th><th>金额</th><th>状态</th><th>更新时间</th><th>文档</th></tr></thead><tbody>'+rows.map(item=>'<tr><td><strong>'+esc(item.title)+'</strong><small>'+esc(item.contract_number)+'</small></td><td>'+esc(item.contract_type||'—')+'</td><td>'+esc(money(item))+'</td><td><span class="status">'+esc(item.status)+'</span></td><td>'+esc(item.updated_at?new Date(item.updated_at).toLocaleDateString('zh-CN'):'—')+'</td><td><a class="link" href="api/v1/contracts/'+encodeURIComponent(item.id)+'/export">导出 DOCX</a></td></tr>').join('')+'</tbody></table>'}
