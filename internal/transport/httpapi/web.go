...
<body>
  ...
  <script>
    ...
    async function initialize(){try{currentUser=await request('api/v1/auth/me');const permissions=currentUser.permissions||[];if(permissions.includes('contract.create')){document.querySelector('#new-contract').classList.remove('hidden');await loadTemplates()}if((currentUser.roles||[]).includes('admin')&&permissions.includes('contract_template.manage'))document.querySelector('#upload-template').classList.remove('hidden')}catch(error){console.error(error)}}
